...
public class PreProcessor {
	...
	public HashSet<Integer> filterDocuments(String document) {
		...
		for (int i = 0; i < listOfFiles.length; i++) {
			if (listOfFiles[i].isFile()) {
				...
				if (fileName.endsWith("aql")) {
					...
					if (printRegexDict)
						System.out.println("AQL file: " + aqlModulePath
								+ aqlModuleName + "/" + fileName);
				}
			}
		}
		...
		if (printRegexDict) {
			System.out.println("total of " + filteredLines.size()
	...
	private void createTempFiles(String document) {
		...
		try {
			...
			while (doc.hasNextLine()) {
				...
				try {
					...
					PrintWriter tempFileWriter = new PrintWriter(
		...
		System.out.println("spliting files takes: " + estimatedTmpeFileTime
				+ " seconds");
	}

	private HashSet<Integer> filterRegex(ArrayList<String> regexList,
			String document) {
		HashSet<Integer> filteredLines = new HashSet<Integer>();
		...
		try {
			// clear index
			Process resetIndex = runtime.exec(new String[] { execIndex,
					"-reset" });
			BufferedReader resetInput = new BufferedReader(
					new InputStreamReader(resetIndex.getInputStream()));
			...
			ProcessBuilder pbIndex = new ProcessBuilder(execIndex,
					goFilesFolder);
			pbIndex.redirectError();
			Process buildIndex = pbIndex.start();
			BufferedReader indexInput = new BufferedReader(
					new InputStreamReader(buildIndex.getInputStream()));
			...
			for (String regex : regexList) {
				...
				ProcessBuilder pbSearch = new ProcessBuilder(execSearch, "-l",
						"-f", goFilesFolder, regex);
				pbSearch.redirectError();
				Process searchRegex = pbSearch.start();
				BufferedReader input = new BufferedReader(
						new InputStreamReader(searchRegex.getInputStream()));
				String inputLine = null;
				while ((inputLine = input.readLine()) != null) {
					try {
						File inputLineFile = new File(inputLine);
						String inputLineAbsolutePath = inputLineFile
								.getCanonicalPath();
						Integer lineNum = Integer
								.parseInt(inputLineAbsolutePath
										.substring(goFilesFolder.length()));
	...
	private HashSet<Integer> filterDictionary(ArrayList<String> dictList,
			String document) {
		HashSet<Integer> filteredLines = new HashSet<Integer>();
		...
		try {
			...
			while (dataFile.hasNextLine()) {
				...
				doc.add(new TextField("data", dataFile.nextLine(),
						Field.Store.YES));
				doc.add(new StringField("id", lineNum.toString(),
						Field.Store.YES));
			...
			try {
				...
				while (dictFile.hasNextLine()) {
					...
					if (!queryStr.isEmpty()) {
						...
						Query query = parser.createPhraseQuery("data",
								QueryParser.escape(queryStr));
	...
	public void setGoFilesFolder(String s) {
		try {
			...
			String goFilesFolderAbsolutePath = goFilesFolderFile
					.getCanonicalPath();
